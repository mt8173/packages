...
public class TabletG9Helper implements OnCheckedChangeListener, OnItemSelectedListener, OnFocusChangeListener {
	...
	public TabletG9Helper(View groupView,int focusUpId) {
	...
	private synchronized void switchFrameIfNeed(int checkedId,String path) {
		...
		if(checkedId==View.NO_ID||TextUtils.isEmpty(path)){
			...
			this.currentId=checkedId;
			this.mRootPath=path;
			if(checkedId==R.id.tablet_game_xiaoji){
				switchXiaoJi(checkedId);
				showLocactionLayout(false);
			}else{
				switchFrameInner(checkedId, path);
				showLocactionLayout(true);
			}
		}
	}
	
	private void showLocactionLayout(boolean b){
		layout.findViewById(R.id.tablet_game_where_layout).setVisibility(b?View.VISIBLE:View.GONE);
	}
	private void switchXiaoJi(int id){
		XLog.i("switchXiaoJi");
		LayoutInflater mInflater=LayoutInflater.from(ctx);
		View layout=mInflater.inflate(R.layout.category_game_center_g9_xiaoji, null);
	...
	private void switchFrameInner(final int id,final String fatherPath){
		...
		new Thread(){
			public void run() {
				...
				LayoutInflater mInflater=LayoutInflater.from(ctx);
				View layout;
				if(id==R.id.tablet_game_arcade){
					layout=mInflater.inflate(R.layout.category_game_center_g9_arcade, null);
					/**
					ImageView view=(ImageView) layout.findViewById(R.id.category_game_arcade_desc);
					ViewGroup.LayoutParams lps=view.getLayoutParams();
					int height=lps.height;
					view.setLayoutParams(lps);
					*/
				}else if(id==R.id.tablet_game_dc||id==R.id.tablet_game_ps1){
					layout=mInflater.inflate(R.layout.category_game_center_g9_location, null);
				}else{
					layout=mInflater.inflate(R.layout.category_game_center_tablet_content, null);
				}
				GridCellLayout view=(GridCellLayout) layout.findViewById(R.id.list_game);
				view.setList(beans);
				view.setMenuId(mFocusUpId);
				if(view.load(id,TabletG9Helper.this)){
					mHandler.obtainMessage(SHOW_CONTENT,id,id, layout).sendToTarget();
				}
			};
		}.start();
	...
	Handler mHandler=new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			case SHOW_LOAD:
				...
				break;
			case SHOW_CONTENT:
				synchronized (TabletG9Helper.this) {
