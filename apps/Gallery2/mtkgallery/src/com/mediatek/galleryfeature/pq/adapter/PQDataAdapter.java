...
public class PQDataAdapter extends BaseAdapter {
    ...
    /**
     * Constructor.
     * @param context getWindowManager by the context.
     * @param uri the image uri.If uri == null means this is an video.
     */
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.left = (TextView) convertView
                    .findViewById(R.id.m_textViewMinValue);
            holder.seekbar = (SeekBar) convertView.findViewById(R.id.m_seekbar);
            holder.blow = (TextView) convertView
                    .findViewById(R.id.m_textViewCurrentIndex);
            holder.right = (TextView) convertView
                    .findViewById(R.id.m_textViewMaxValue);
            holder.layout = (RelativeLayout) convertView
                    .findViewById(R.id.m_listitem);
    ...
    private void setItemHeight(ViewHolder holder, int count) {
        ...
        if (sceenHeigh == 0) {
            sceenHeigh = ((Activity) mContext).getWindowManager()
                    .getDefaultDisplay().getHeight()
                    - ((PictureQualityActivity) mContext).getActionBarHeight();
        }
        int defaultItemH = ((PictureQualityActivity) mContext)
                .getDefaultItemHeight();
        int itemHeight;
        if (count * defaultItemH < sceenHeigh) {
            itemHeight = (sceenHeigh - ((PictureQualityActivity) mContext)
                    .getActionBarHeight()) / count;
        } else {
