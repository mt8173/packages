...
public class DeviceInfoSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    //MTK system update feature
    private static final String KEY_MTK_SYSTEM_UPDATE = "mtk_system_update";
    private static final String PROPERTY_MTK_UPDATE_SETTING = "ro.mtk_system_update_support";

    static final int TAPS_TO_BE_A_DEVELOPER = 7;
	
	private static final String CUSTOM_MODEL_NUMBER = SystemProperties.get("ro.wisky.modelnumber", "Android");
	private static final String CUSTOM_VERSION_PRE_STRING = "V"+SystemProperties.get("ro.product.firmware", "1.00")+"_";
	private static final String CUSTOM_VERSION_BACK_STRING = SystemProperties.get("ro.wisky.build_number_two", "");
	private static final String CUSTOM_DATE_FORMAT = SystemProperties.get("persist.wisky.dateformat", "yyyyMMdd");
	//xiaoyuguang 20140604 add hardware info 
	private static final String KEY_DEVICE_HARDWAREINFO = "device_hardwareinfo"; 
	private static final String CUSTOM_DEVICE_HARDWAREINFO = SystemProperties.get("custom.hardwareinfo","yes");
	//end xiaoyuguang 20140604

    long[] mHits = new long[3];
    int mDevHitCountdown;
    int mFactoryToolsCountdown;
    Toast mDevHitToast;
    ...
    public void onCreate(Bundle icicle) {
		...
		//setStringSummary(KEY_DEVICE_MODEL, CUSTOM_MODEL_NUMBER);

		//SimpleDateFormat simpleDateFormat = new SimpleDateFormat(CUSTOM_DATE_FORMAT);
		//String revision = CUSTOM_VERSION_PRE_STRING + simpleDateFormat.format(new Date(Build.TIME)) + CUSTOM_VERSION_BACK_STRING;
		//setStringSummary(KEY_BUILD_NUMBER, revision);
		
        findPreference(KEY_BUILD_NUMBER).setEnabled(true);
        findPreference(KEY_KERNEL_VERSION).setSummary(DeviceInfoUtils.getFormattedKernelVersion());
		findPreference(KEY_KERNEL_VERSION).setEnabled(true);
		findPreference(KEY_CHECK_UPDATE).setEnabled(true);
        ...
        /// M: System and software update. @start{
        if (!mUm.isAdminUser()
                  || !SystemProperties.get(PROPERTY_MTK_UPDATE_SETTING).equals("1")) {
             removePreference(KEY_MTK_SYSTEM_UPDATE);
        } else {
            // M: if this entrance exists ,remove the google default OTA entrance
            removePreference(KEY_SYSTEM_UPDATE_SETTINGS);
            Log.d(LOG_TAG, "reomve the google default OTA entrance for system updates");
            updateTitleToActivityLabel(KEY_MTK_SYSTEM_UPDATE);
        }
       /// @ end}
	   
		//xiaoyuguang 20140604 add hardware info
		if("no".equals(CUSTOM_DEVICE_HARDWAREINFO)){
			getPreferenceScreen().removePreference(findPreference(KEY_DEVICE_HARDWAREINFO));
		}
    ...
    public void onResume() {
		...
		mFactoryToolsCountdown = TAPS_TO_BE_A_DEVELOPER;
        mDevHitToast = null;
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        if (preference.getKey().equals(KEY_FIRMWARE_VERSION)) {
            ...
            //start wisky factoryTestTools
			if (mFactoryToolsCountdown > 0) {
                mFactoryToolsCountdown--;
                if (mFactoryToolsCountdown == 0) {
					//start activity
					startActivity("com.wisky.factorytools","com.wisky.factorytools.MainActivity");
					//end
                }
            }
			//end
        ...
        } else if (preference.getKey().equals(KEY_MTK_SYSTEM_UPDATE)) {
            if (SystemProperties.get(PROPERTY_MTK_UPDATE_SETTING).equals("1")) {
                startActivity("com.mediatek.systemupdate",
                        "com.mediatek.systemupdate.MainEntry");
            }
        }else if(preference.getKey().equals(KEY_CHECK_UPDATE)){
			//by yanghao
			try {
			 Intent intent = new Intent();
			 ComponentName component = new ComponentName("com.wisky.updater", "com.wisky.updater.UpdateActivity");  
       ...
       if (intent != null) {
            ...
            for (int i = 0; i < listSize; i++) {
                ...
                if ((resolveInfo.activityInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM)
                    ...
                    Log.d(LOG_TAG, "KEY_MTK_SYSTEM_UPDATE : " + title);
                    break;
                }
            }
        }
