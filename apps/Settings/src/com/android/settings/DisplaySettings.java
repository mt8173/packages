...
public class DisplaySettings extends SettingsPreferenceFragment implements
	...
	private static final String KEY_NET_WATCH = "net_watch";
	public final String SHOW_NETSPEED = "show_netspeed";
	...
	private SwitchPreference mNetWatch;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		mNetWatch = (SwitchPreference) findPreference(KEY_NET_WATCH);
		mNetWatch.setOnPreferenceChangeListener(this);
        ...
        if (mHdmiManager != null && SystemProperties.get("ro.wisky.hdmi","true").equals("true")) {
            mHdmiSettings = new Preference(activity);
            mHdmiSettings.setKey(KEY_HDMI_SETTINGS);
            mHdmiSettings.setTitle(R.string.hdmi_settings);
            mHdmiSettings.setSummary(R.string.hdmi_settings_summary);
            mHdmiSettings.setFragment("com.mediatek.hdmi.HdmiSettings");
            try {
                String hdmi = activity.getString(R.string.hdmi_replace_hdmi);
                if (mHdmiManager.getDisplayType() == HdmiDef.DISPLAY_TYPE_MHL) {
                    String mhl = activity.getString(R.string.hdmi_replace_mhl);
                    ...
                    String slimport = activity.getString(R.string.slimport_replace_hdmi);
                    mHdmiSettings.setTitle(mHdmiSettings.getTitle().toString()
            ...
            getPreferenceScreen().addPreference(mHdmiSettings);
        }
        // @ }
        // add for MiraVision @ {
        mMiraVision = new Preference(activity);
        mMiraVision.setKey(KEY_MIRA_VISION);
        mMiraVision.setTitle(R.string.mira_vision_title);
        mMiraVision.setSummary(R.string.mira_vision_summary);
        mMiraVision.setOrder(PREFERENCE_ORDER_FIRST + 1);
        getPreferenceScreen().addPreference(mMiraVision);

        List<ResolveInfo> apps = activity.getPackageManager().queryIntentActivities(mMiraIntent, 0);
        if (apps == null || apps.size() == 0) {
            Log.d(TAG, "No MiraVision apk");
            getPreferenceScreen().removePreference(mMiraVision);
        }
        if (!SystemProperties.get("ro.mtk_miravision_support").equals("1")) {
            Log.d(TAG, "No MiraVision lib so");
            getPreferenceScreen().removePreference(mMiraVision);
        }
        if (android.os.UserHandle.myUserId() != 0 && SystemProperties.get("ro.build.characteristics").equals("tablet")) {
            Log.d(TAG, "Only the owner can see MiraVision Settings");
            getPreferenceScreen().removePreference(mMiraVision);
        }
		//removePreference("screen_zoom");
        // @ }
    }
    ...
    private void updateState() {
		...
		if(mNetWatch!=null){
			mNetWatch.setChecked(Settings.Global.getInt(getContentResolver(),
			SHOW_NETSPEED, 0) != 0);	
		}	
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
		...
		if (preference == mNetWatch) {
            boolean value = (Boolean) objValue;
            Settings.Global.putInt(getContentResolver(), SHOW_NETSPEED, value?1:0);
        }
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
       if (preference == mMiraVision) {
            // add for MiraVision
            getActivity().startActivity(mMiraIntent);
        }
