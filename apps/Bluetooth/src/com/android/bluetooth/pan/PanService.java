...
public class PanService extends ProfileService {
    ...
    private static final int MIN_CONNECTED_PAN_DEVICE = 1;
    ...
    void handlePanDeviceStateChange(BluetoothDevice device,
        ...
        if (remote_role == BluetoothPan.LOCAL_PANU_ROLE) {
            if (state == BluetoothProfile.STATE_CONNECTED) {
                ...
                if (getConnectedPanDevices().size() < MIN_CONNECTED_PAN_DEVICE){
                    Log.d(TAG, "[handlePanDeviceStateChange]ConnectedPanDevices:" +
                              getConnectedPanDevices().size());
